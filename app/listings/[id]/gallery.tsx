...
export function Gallery({ photos, title }: { photos: string[]; title: string }) {
  ...
  return (
    <div className="space-y-3">
      <Image src={safePhotos[index]} alt={`${title} photo ${index + 1}`} width={900} height={600} className="h-64 w-full rounded-lg object-cover sm:h-80" />
      <div className="flex items-center gap-2 overflow-x-auto">
        {safePhotos.map((photo, i) => (
          <button key={photo + i} onClick={() => setIndex(i)} className={`overflow-hidden rounded border ${i === index ? "ring-2 ring-black" : ""}`}>
            <Image src={photo} alt={`Thumb ${i + 1}`} width={80} height={64} className="h-16 w-20 object-cover" />
          </button>
        ))}
      </div>
